...
def comparer_plot2(
    ...
    plt.plot(
        mf1.m,
        mf1.dndm / mf2.dndm - 1,
        label=f"{varying_param} = {eval(varying_param)} ($\Delta = 500c$)",
        linestyle="--",
        color=color,
    )
...
def comparer_z2(
    ...
    comparer_plot(nom1, nom2, mdef_model, overdensity, Om0, 0, Mmin, "z", "blue")
    comparer_plot(nom1, nom2, mdef_model, overdensity, Om0, 0.7, Mmin, "z", "orange")
    comparer_plot(nom1, nom2, mdef_model, overdensity, Om0, 1.4, Mmin, "z", "green")
    # comparer_plot2(
    #     "Bocquet500cHydro",
    #     "Bocquet500cDMOnly",
    #     mdef_model,
    ...
    # )
    plt.xscale("log")
    plt.xlabel(r"Masse ($h^{-1} M_\odot$)", size=12)
    plt.ylabel(r"$\frac{dn}{dM}$", size=18)
    plt.legend()
